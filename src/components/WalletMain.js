// src/components/WalletMain.js - 토큰 영구 저장 기능 연동
...
import { 
  ...
  Plus, Search, ChevronDown, ChevronUp, Coins, X, CheckCircle, Lock, ChevronLeft, MessageCircle
} from 'lucide-react';
import LanguageToggle from './LanguageToggle';
import { walletService } from '../services/walletService';
import { MultiWalletManager } from '../services/MultiWalletManager';
import { SecureStorage } from '../services/secureStorage'; // SecureStorage 추가
...
const WalletMain = ({ 
  ...
  const [selectedAsset, setSelectedAsset] = useState(null); // 선택된 자산 (WLC 또는 토큰)
  ...
  useEffect(() => {
    // 화면 로드 시 및 network 변경 시 walletData 출력 
  }, [network, walletData]);
  ...
  }, [network, walletData, walletPassword]); 

  // 지갑 초기화 - 저장된 토큰 로드 및 자동 토큰 발견
  const initializeWallet = async () => {
    // 필수 조건 확인
    if (!walletData?.id || !walletData?.address || !walletPassword) {
      consolewarn(t('WalletMain.walletInitFailed'), {
    ...
    consolelog(t('WalletMain.tokenAddStarted'), walletData.id);
    
    try {
      // 저장된 토큰 목록 로드 (비밀번호 필요)
      const savedTokens = MultiWalletManager.getWalletTokens(walletData.id, walletPassword);
      if (savedTokens.length > 0) {
        setTokens(savedTokens);
        consolelog(`저장된 토큰 ${savedTokens.length}개 로드됨`);
      }

      // 통합 새로고침 실행 (토큰 발견 + 토큰 목록 갱신)
      await refreshAllData(false);
    } catch (error) {
      consoleerror('지갑 초기화 실패:', error);
  ...
  useEffect(() => {
    ...
    const events = [
      ...
      'click', 'focus', 'input', 'change', 'wheel', 'touchmove' // 추가
    ];
  ...
  const loadBannerAds = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        bannerData.url = '/sites.html';  // bannerData.url을 소스코드에 하드코딩하여 수정하지 못하게 함.
        
        // URL들을 PUBLIC_URL을 고려하여 처리
        const processedBannerData = {
          ...bannerData,
          // 이미지 경로가 상대 경로인 경우 PUBLIC_URL 추가
          imageSrc: bannerData.imageSrc && !bannerData.imageSrc.startsWith('http') && !bannerData.imageSrc.startsWith('//')
            ...
            : bannerData.imageSrc,
          // 링크 URL이 상대 경로인 경우 PUBLIC_URL 추가
        ...
        consolelog(t('WalletMain.bannerAdsLoaded'), processedBannerData);
      } else {
        consolewarn('배너 광고 파일을 찾을 수 없습니다', response.status);
      }
    } catch (error) {
      consoleerror('배너 광고 로드 실패:', error);
    } finally {
  ...
  const loadInfoUrls = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        consolelog(t('WalletMain.boardUrlsLoaded'), processedUrls.length);
      } else {
        consolewarn('게시판 URL 파일을 찾을 수 없습니다', response.status);
      }
    } catch (error) {
      consoleerror('게시판 URL 로드 실패:', error);
    } finally {
  ...
  const discoverAndLoadTokens = async () => {
    if (!walletData?.address || !walletData?.id || !walletPassword) {
      consolewarn(t('WalletMain.noTokensFound'));
      return;
    }

    try {
      consolelog(t('WalletMain.tokenInfoQuerying'), walletData.address);
      setIsLoading(true);

      // WorldLand Explorer API를 통해 이 지갑의 모든 토큰 잔액 조회
      ...
      if (result.success) {
        ...
        if (result.native) {
          ...
          consolelog('WLC 잔액:', result.native.balance);
        }
        ...
        if (result.tokens && result.tokens.length > 0) {
            ...
            if (existingAddresses.has(tokenAddress)) {
              ...
              if (balance > 0) {
                ...
                if (addResult.success) {
                  newTokensAdded++;
                  consolelog(`새 토큰 자동 추가: ${discoveredToken.symbol} (잔액: ${discoveredToken.balance})`);
                }
              }
            }
          }
          ...
          consolelog(`토큰 발견 완료: 신규 ${newTokensAdded}개, 업데이트 ${updatedTokensCount}개`);
        } else {
          consolelog(t('WalletMain.noTokensFound'));
        }
      } else {
        consolewarn('통합 잔액 조회 실패, 개별 조회로 대체');
        
        // API 실패 시 기존 방식으로 네이티브 잔액만 조회
        const nativeResult = await walletService.getBalance(walletData.address);
        if (nativeResult.success) {
          setBalance(nativeResult.balance);
        }

        // 기존 저장된 토큰들의 잔액만 업데이트
        const savedTokens = MultiWalletManager.getWalletTokens(walletData.id, walletPassword);
        if (savedTokens.length > 0) {
          ...
          if (tokenUpdateResult.success) {
            ...
            consolelog(`${t('WalletMain.tokenBalanceUpdated')} ${tokenUpdateResult.updatedCount}개`);
          }
        }
      }
    } catch (error) {
      consoleerror('자동 토큰 발견 실패:', error);
      
      // 오류 발생 시에도 기본 잔액은 조회 시도
      try {
        const nativeResult = await walletService.getBalance(walletData.address);
        if (nativeResult.success) {
          setBalance(nativeResult.balance);
        ...
        consoleerror('네이티브 잔액 조회도 실패:', balanceError);
      }
    } finally {
      setIsLoading(false);
    }
  ...
  // 통합 새로고침 함수 (토큰 발견 + 토큰 목록 갱신)
  const refreshAllData = async (isAutoRefresh = false) => {
    try {
      consolelog(t('WalletMain.dataRefreshComplete'));
      
      // 1. 토큰 발견 및 잔액 업데이트
      await discoverAndLoadTokens();
      
      // 2. 토큰 목록 갱신 (수동 새로고침이거나 5분마다)
      const shouldUpdateTokenList = !isAutoRefresh || 
        !lastTokenListUpdate || 
        (Date.now() - lastTokenListUpdate) > 5 * 60 * 1000; // 5분
      
      if (shouldUpdateTokenList) {
        consolelog(t('WalletMain.popularTokensLoading'));
      ...
      consoleerror('통합 데이터 새로고침 실패:', error);
      throw error;
    }
  };

  // 수동 토큰 발견 (새로고침 버튼 클릭 시)
  const handleManualTokenDiscovery = async () => {
    ...
    try {
      ...
      await refreshAllData(false); // 토큰 목록도 함께 갱신
      // 자동 새로고침 재시작
      startAutoRefresh();
    } catch (error) {
  ...
  const startAutoRefresh = () => {
    ...
    // 5분마다 자동 새로고침 (토큰 발견 + 토큰 목록 갱신 포함)
    const interval = setInterval(() => {
      consolelog(t('WalletMain.autoBalanceUpdate'));
      setIsAutoRefreshing(true);
      refreshAllData(true).finally(() => { // 자동 새로고침은 토큰 목록을 5분마다만 갱신
    ...
    setLastUpdateTime(new Date());
    consolelog(t('WalletMain.autoRefreshStarted'));
  };
  ...
  const stopAutoRefresh = () => {
    if (autoRefreshInterval) {
      ...
      consolelog(t('WalletMain.autoRefreshStopped'));
    }
  };

  // 수동 새로고침 (자동 새로고침 재시작 + 토큰 발견 + 토큰 목록 갱신)
  const handleManualRefresh = async () => {
    setIsLoading(true);
    try {
      // 통합 새로고침 실행 (토큰 목록도 갱신)
      await refreshAllData(false);
      
      // 자동 새로고침 재시작
      startAutoRefresh();
      
      // 성공 메시지는 discoverAndLoadTokens에서 이미 처리됨
      if (!success) {
  ...
  const loadPopularTokens = async () => {
    try {
      ...
      if (result.success) {
        ...
        consolelog(`인기 토큰 ${result.tokens.length}개 로드됨`);
      } else {
        consolewarn('인기 토큰 로드 실패:', result.error);
      }
    } catch (error) {
      consolewarn('인기 토큰 로드 예외:', error);
    }
  };
  ...
  // loadAllBalances 함수 수정 - 비밀번호 매개변수 추가
  const loadAllBalances = async () => {
    if (!walletData?.address || !walletData?.id || !walletPassword) {
      consolewarn(t('WalletMain.balanceUpdateFailed'));
      return;
    }
    
    // 수동 새로고침이 아닌 경우에만 로딩 상태 설정
    if (!isAutoRefreshing) {
      setIsLoading(true);
    }
    
    try {
      consolelog(t('WalletMain.balanceQueryStarted'), walletData.address);
      ...
      if (result.success) {
        ...
        if (savedTokens.length > 0) {
          // 저장된 토큰들의 잔액을 API에서 조회된 데이터로 업데이트
          const updatedTokens = savedTokens.map(savedToken => {
            ...
            if (foundToken) {
              ...
              // MultiWalletManager에도 업데이트된 잔액 저장 (비밀번호 포함)
            ...
            return savedToken; // API에서 찾지 못한 경우 기존 정보 유지
          });
          
          setTokens(updatedTokens);
          consolelog(`저장된 토큰 ${updatedTokens.length}개 잔액 업데이트됨`);
        }
        
        // API에서 새로 발견된 토큰들 중 잔액이 0보다 큰 것들 자동 추가
        if (result.tokens && result.tokens.length > 0) {
          const existingAddresses = new Set(
          ...
          const newTokens = result.tokens.filter(token => 
            ...
            parseFloat(token.balance) > 0 // 잔액이 0보다 큰 토큰만
          );
          ...
          for (const newToken of newTokens) {
            ...
            if (addResult.success) {
              consolelog(`새 토큰 자동 추가: ${newToken.symbol}`);
            }
          }
          
          // 새로 추가된 토큰이 있으면 상태 업데이트
        ...
        // 자동 새로고침 중이 아닐 때만 성공 메시지
        if (!isAutoRefreshing) {
          consolelog('잔액 업데이트 완료');
        }
      } else {
        // 실패 시 기존 방식으로 네이티브 잔액만 조회
        const nativeResult = await walletService.getBalance(walletData.address);
        if (nativeResult.success) {
          setBalance(nativeResult.balance);
        }
        
        // 저장된 토큰들의 잔액을 개별적으로 업데이트
        const savedTokens = MultiWalletManager.getWalletTokens(walletData.id, walletPassword);
        if (savedTokens.length > 0) {
          const tokenUpdateResult = await MultiWalletManager.updateAllTokenBalances(
            walletData.id, 
          ...
          if (tokenUpdateResult.success) {
            setTokens(tokenUpdateResult.tokens);
            consolelog(`개별 토큰 잔액 업데이트: ${tokenUpdateResult.updatedCount}개`);
          }
        }
        
        if (!isAutoRefreshing) {
          consolewarn('통합 조회 실패, 개별 조회로 대체');
        }
      }
    } catch (error) {
  ...
  // 보안 강화된 handleAddToken 함수 - 완전한 수정본
  const handleAddToken = async (token) => {
    // 1. 기본 검증 - 지갑 정보와 비밀번호 확인
    if (!walletData?.id) {
      setError(t('WalletMain.authRequired'));
      consoleerror('지갑 데이터 누락');
      return;
    }

    if (!walletPassword) {
      setError(t('WalletMain.authRequired'));
      consoleerror('지갑 비밀번호 누락');
      return;
    }

    consolelog(t('WalletMain.tokenAddStarted'), { walletId: walletData.id, token });
    ...
    if (!token) {
      ...
      setIsLoading(true);
      try {
        consolelog(t('WalletMain.tokenInfoQuerying'), normalizedAddress);
        
        // 5. 토큰 정보 조회
        const result = await walletService.getTokenBalance(walletData.address, normalizedAddress);
        
        if (result.success) {
          consolelog('토큰 정보 조회 성공:', result.token);
          
          // 6. MultiWalletManager에 토큰 추가 (암호화 저장)
          const addResult = MultiWalletManager.addTokenToWallet(
            walletData.id, 
            result.token, 
            walletPassword
          ...
          consolelog('토큰 추가 결과:', addResult);
          
          if (addResult.success) {
            // 7. 상태 업데이트 - 암호화된 저장소에서 토큰 목록 다시 로드
            const updatedTokens = MultiWalletManager.getWalletTokens(walletData.id, walletPassword);
            consolelog('업데이트된 토큰 목록:', updatedTokens);
      ...
      setIsLoading(true);
      try {
        consolelog('브라우저 토큰 추가 시작:', token);
        ...
        // 12. MultiWalletManager에 토큰 추가 (암호화 저장)
        const addResult = MultiWalletManager.addTokenToWallet(
          walletData.id, 
          sanitizedToken, 
          walletPassword
        ...
        consolelog('브라우저 토큰 추가 결과:', addResult);
        
        if (addResult.success) {
          // 13. 토큰 잔액 조회 시도 (백그라운드에서)
          try {
            consolelog(t('WalletMain.balanceQueryStarted'), sanitizedToken.address);
            const balanceResult = await walletService.getTokenBalance(walletData.address, sanitizedToken.address);
            
            if (balanceResult.success) {
              consolelog('토큰 잔액 조회 성공:', balanceResult.token);
              // 14. 잔액 정보로 토큰 업데이트 (암호화 저장)
              const updateResult = MultiWalletManager.updateTokenBalance(
                walletData.id,
              ...
              if (!updateResult.success) {
                consolewarn('토큰 잔액 업데이트 실패:', updateResult.error);
              }
            } else {
              consolewarn('토큰 잔액 조회 실패, 기본값 사용:', balanceResult);
            }
          } catch (balanceError) {
            consolewarn('토큰 잔액 조회 예외, 기본값 사용:', balanceError);
          }
          
          // 15. 상태 업데이트 - 암호화된 저장소에서 최신 토큰 목록 로드
          const updatedTokens = MultiWalletManager.getWalletTokens(walletData.id, walletPassword);
          consolelog('최종 업데이트된 토큰 목록:', updatedTokens);
          ...
          consoleerror('브라우저 토큰 추가 실패:', addResult);
          setError(addResult.error || t('WalletMain.tokenAddFailed'));
        }
      } catch (error) {
        consoleerror('브라우저 토큰 추가 예외:', error);
        setError(t('WalletMain.tokenAddFailed') + ': ' + error.message);
      } finally {
        setIsLoading(false);
      }
  ...
  const handleRemoveToken = (tokenAddress) => {
    if (!walletData?.id || !walletPassword) {
      consoleerror('토큰 제거 실패: 지갑 정보나 비밀번호 누락');
    ...
    if (window.confirm(t('WalletMain.confirmRemoveToken'))) {
      try {
        ...
        if (removeResult.success) {
          // 상태 업데이트 - 암호화된 저장소에서 토큰 목록 다시 로드
          const updatedTokens = MultiWalletManager.getWalletTokens(walletData.id, walletPassword);
          setTokens(updatedTokens);
          setSuccess(t('WalletMain.tokenRemoved'));
          
          // 제거된 토큰이 현재 선택된 자산이면 WLC로 변경
          const currentNetwork = walletService.getCurrentNetwork();
          ...
          consoleerror('토큰 제거 실패:', removeResult.error);
          setError(removeResult.error || t('WalletMain.tokenRemoveFailed'));
          setTimeout(() => setError(''), 3000);
        }
      } catch (error) {
        consoleerror('토큰 제거 예외:', error);
  ...
  const filteredAssets = allAssets.filter(asset => {
    // 검색어 필터
    const matchesSearch = asset.symbol.toLowerCase().includes(searchToken.toLowerCase()) ||
                        asset.name.toLowerCase().includes(searchToken.toLowerCase());
     
    // 네트워크 필터: native 토큰은 항상 표시, 일반 토큰은 network가 일치하는 것만 표시
    const matchesNetwork = asset.type === 'native' || asset.network === currentNetwork.name;
  ...
  const handleDappButtonClick = () => {
    consolelog('🔍 DApp 버튼 클릭');
    consolelog('🔍 isDummy:', walletData?.isDummy);
    consolelog('🔍 address:', walletData?.address);
    
    // 더미 지갑이거나 주소가 없는 경우
    if (walletData?.isDummy || !walletData?.address || walletData.address === '0x0000000000000000000000000000000000000000') {
      alert('\n"월드랜드 시작하기" 또는 "지갑 열기" 버튼을 클릭하여 지갑을 열어주세요.');
      return;
    }
    
    // 정상 지갑인 경우 DApp 화면 표시
    setShowDapp(true);
  };
  ...
  const formatLastUpdateTime = () => {
    ...
    const now = new Date();
    const diff = Math.floor((now - lastUpdateTime) / 1000); // 초 단위
    ...
    return (
      <div className="wallet-main">
        <div className="main-header">
          ...
          {/* <ChevronLeft size={24} onClick={() => setCurrentView('main')} style={{cursor: 'pointer'}} /> */}
          {/* <div 
  ...
  const getWalletButtonText = () => {
    if (walletData?.isDummy && !hasWallet) {
      ...
      return t('WalletMain.settings'); // 이미 정의되어 있음
    }
  };
  ...
  const handleSendClick = () => {
    // 더미 지갑이거나 주소가 없는 경우 체크
    if (walletData?.isDummy || 
        !walletData?.address || 
        walletData.address === '0x0000000000000000000000000000000000000000') {
      alert('\n"월드랜드 시작하기" 또는 "지갑 열기" 버튼을 클릭하여 지갑을 열어주세요.');
      return; // 여기서 함수 종료 (화면 전환 안 함)
    }
    
    // 정상적인 경우에만 송금 화면으로 이동
    setCurrentView('send');
  };
    ...
    <div className="wallet-main" >
      ...
      <div className="wallet-header">
        ...
        <div className="header-actions">  
          ...
          <button className="action-btn" onClick={onLock} title={t("wallet.lockWallet")}>
            <Lock size={16} />
          </button>  
        </div>
      </div>
      ...
      {/* <LanguageToggle /> */}
        ...
        <div className="dapp-button-container">
          <button 
            ...
            onClick={handleDappButtonClick}  // 👈 수정됨
        ...
        >
          <div className="asset-info">
            <div className="asset-icon">
              {selectedAsset?.type === 'native' ? (selectedAsset?.emoji || '🏆') : '🪙'}
            </div>
                ...
                >
                  <div className="asset-info" >
                    <div className="asset-icon" onClick={(e) => {
                      ...
                      {asset.type === 'native' ? (selectedAsset?.emoji || '🏆') : '🪙'}
                    </div>
        ...
        <div className="modal-overlay">
          <div className="modal-content token-browser-modal">
            <div className="modal-header">
              ...
              <button onClick={() => setShowTokenBrowser(false)}>×</button>
            </div>
        ...
        <div className="modal-overlay">
          <div className="modal-content add-token-modal">
            <div className="modal-header">
              ...
              <button onClick={() => setShowAddToken(false)}>×</button>
            </div>
        ...
        <div className="modal-overlay">
          <div className="modal-content token-detail-modal">
            <div className="modal-header">
              ...
              <button onClick={handleCloseTokenDetail}>×</button>
            </div>
              ...
              <div className="token-detail-info">
                ...
                {selectedTokenDetail.verified && (
                  <div className="detail-row">
                    ...
                    <span className="value verified">✓ {t('WalletMain.verified')}</span>
                  </div>
                )}
              </div>
              ...
              <div className="external-links">
                ...
                <div className="link-buttons">
                  ...
                  <button 
                    ...
                    onClick={() => {
                      const currentNetwork = walletService.getCurrentNetwork();
                      window.open(`${currentNetwork.explorer}/token/${selectedTokenDetail.address}`, '_blank');
                    }}
                  >
                    📊 {t('WalletMain.tokenInfoView')}
                  </button>
                  <button 
                    className="external-link-btn"
                    onClick={() => {
                      const currentNetwork = walletService.getCurrentNetwork();
                      window.open(`${currentNetwork.explorer}/token/${selectedTokenDetail.address}/token-transfers`, '_blank');
                    }}
                  >
                    📈 {t('WalletMain.tokenTransferHistory')}
                  </button>
                  <button 
                    className="external-link-btn"
                    onClick={() => {
                      const currentNetwork = walletService.getCurrentNetwork();
                      window.open(`${currentNetwork.explorer}/address/${walletData.address}/tokens`, '_blank');
                    }}
                  >
                    💼 {t('WalletMain.allWalletTokens')}
                  </button>
                </div>
              </div>
            </div>
     ...
     <div className="token-main-info">
       ...
       <div className="token-details">
         <div className="token-name-row">
           ...
           {token.verified && <span className="verified-badge">✓</span>}
         </div>
