...
class TransactionCache {
  ...
  // cacheKey는 이제 "address_network" 형식을 받음
  getCachedTransactions(cacheKey) {
    try {
      const cached = localStorage.getItem(`${this.cacheKey}_${cacheKey}`);
  ...
  setCachedTransactions(cacheKey, transactions) {
    try {
      ...
      localStorage.setItem(`${this.cacheKey}_${cacheKey}`, JSON.stringify(data));
    } catch (error) {
...
class WorldLandExplorerAPI {
  constructor(network = 'testnet') {
    ...
    // 네트워크 설정에서 explorer URL을 가져옵니다
    const networkConfig = NETWORKS[network] || NETWORKS.mainnet;
    this.baseUrl = networkConfig.explorer;
  }
  ...
  async getAccountTransactions(address, options = {}) {
    try {
      ...
      // WorldLand API 응답 형식: {"message":"OK","result":[...]}
...
class EtherscanAPI {
  constructor(network = 'ethereum') {
    ...
    // 실제 사용 시에는 Etherscan API 키가 필요합니다
    // 무료: 초당 5개 요청 제한
    this.apiKey = 'YourApiKeyToken'; // 실제 API 키로 교체 필요
  }
  ...
  async getAccountBalance(address) {
    try {
      ...
      if (!response.ok) {
        throw new Error(`Etherscan API 요청 실패: ${response.status}`);
      }
      ...
      consolewarn('Etherscan 잔액 조회 실패:', error);
      return {
        success: false,
        error: error.message
      };
  ...
  async getAccountTransactions(address, options = {}) {
    try {
      const { offset = 20, page = 1 } = options;
      ...
      consolelog('Etherscan API 요청:', url);
      ...
      consoleerror('Etherscan 거래내역 조회 실패:', error);
      return {
        success: false,
        transactions: [],
        error: error.message
  ...
  async getAllTransactions(address, options = {}) {
    try {
      ...
      // 토큰 트랜잭션이 요청된 경우
      if (tokenAddress) {
        return await this.getTokenTransactions(address, tokenAddress, limit);
      }

      // 일반 트랜잭션 조회
      const normalTxResult = await this.getAccountTransactions(address, { offset: limit });
      
      if (!normalTxResult.success) {
        return normalTxResult;
      }

      // ERC-20 토큰 트랜잭션 조회
      const tokenTxResult = await this.getTokenTransfers(address, limit);
      
      // 두 결과 합치기
      const allTransactions = [
        ...normalTxResult.transactions,
        ...(tokenTxResult.success ? tokenTxResult.transactions : [])
      ];
      ...
      return {
        success: true,
        transactions: allTransactions.slice(0, limit),
        source: 'etherscan-all'
      };

    } catch (error) {
      consoleerror('Etherscan 통합 거래내역 조회 실패:', error);
      return {
        success: false,
        transactions: [],
        error: error.message
  ...
  async getTokenTransfers(address, limit = 20) {
    try {
      const url = `${this.baseUrl}?module=account&action=tokentx&address=${address}&startblock=0&endblock=99999999&page=1&offset=${limit}&sort=desc&apikey=${this.apiKey}`;
      ...
      if (!response.ok) {
        throw new Error(`Token transfers API 실패: ${response.status}`);
      }
      ...
      consolewarn('Token transfers 조회 실패:', error);
      return {
        success: false,
        transactions: [],
        error: error.message
  ...
  async getTokenTransactions(address, tokenAddress, limit = 20) {
    try {
      const url = `${this.baseUrl}?module=account&action=tokentx&contractaddress=${tokenAddress}&address=${address}&startblock=0&endblock=99999999&page=1&offset=${limit}&sort=desc&apikey=${this.apiKey}`;
      ...
      if (!response.ok) {
        throw new Error(`Token transactions API 실패: ${response.status}`);
      }
      ...
      if (data.status === '1' && data.result && Array.isArray(data.result)) {
        ...
        return {
          success: true,
          transactions: transactions,
          source: 'etherscan-tokentx-filtered'
        };
      }
      ...
      consoleerror('특정 토큰 거래내역 조회 실패:', error);
      return {
        success: false,
        transactions: [],
        error: error.message
  ...
  async getAccountTokenBalances(address) {
    // Etherscan에서는 토큰 잔액을 직접 조회하는 API가 제한적
    // tokenBalance 엔드포인트를 사용하거나, 알려진 토큰 목록에 대해 개별 조회 필요
    return {
      success: false,
      tokens: [],
      error: 'Etherscan API는 전체 토큰 잔액 조회를 지원하지 않습니다'
    };
  }
  ...
  async getTokenBalance(address, tokenAddress) {
    try {
      ...
      if (!response.ok) {
        throw new Error(`Token balance API 실패: ${response.status}`);
      }

      const data = await response.json();
      
      if (data.status === '1' && data.result) {
        // 토큰 정보는 별도로 조회해야 함 (컨트랙트 ABI 사용)
        return {
          success: true,
          token: {
      ...
      throw new Error(data.message || 'Token balance query failed');

    } catch (error) {
      consolewarn('Token balance 조회 실패:', error);
      return {
        success: false,
        error: error.message
      };
...
// BscScan API 클래스 (Etherscan과 거의 동일한 API 구조)
class BscScanAPI extends EtherscanAPI {
  constructor(network = 'bsc') {
    super(network);
    this.network = network;
    this.baseUrl = 'https://api.bscscan.com/api';
    this.apiKey = 'YourApiKeyToken'; // 실제 BscScan API 키로 교체 필요
  }
}
...
export class WalletService {
  ...
  async getTransactionHistory(address, limit = 20, useCache = true, selectedAsset = null, network = null) {
    try {
      // 네트워크가 명시적으로 전달되지 않으면 현재 네트워크 사용
      const targetNetwork = network || this.network;
      consolelog(`통합 거래내역 조회 시작: ${address}, 네트워크: ${targetNetwork}`);

      // 네트워크별로 캐시 키 생성
      const cacheKey = `${address}_${targetNetwork}`;
      
      if (useCache) {
        const cached = this.cache.getCachedTransactions(cacheKey);
        if (cached && cached.length > 0) {
          consolelog(`캐시에서 거래내역 반환 (${targetNetwork})`);
          let filtered = cached;
      ...
      // 네트워크가 변경된 경우, explorer도 변경
      const originalNetwork = this.network;
      if (targetNetwork !== this.network) {
        this.switchNetwork(targetNetwork);
      }
      ...
      try {
        ...
        if (allTransactionsResult.success && allTransactionsResult.transactions.length > 0) {
          consolelog(`통합 API 성공 (${targetNetwork}): ${allTransactionsResult.transactions.length}개 거래`);
          this.cache.setCachedTransactions(cacheKey, allTransactionsResult.transactions);
          return {
        ...
        consolelog(`네이티브 거래만 조회 중 (${targetNetwork})...`);
        ...
        if (nativeResult.success && nativeResult.transactions.length > 0) {
          consolelog(`네이티브 거래 성공 (${targetNetwork}): ${nativeResult.transactions.length}개 거래`);
          this.cache.setCachedTransactions(cacheKey, nativeResult.transactions);
          return {
            ...nativeResult,
            source: 'native-only',
            network: targetNetwork
        ...
        console.log(`실제 API 실패 (${targetNetwork}), 데모 데이터 사용`);
        return await this.getDemoTransactions(address, limit);
      } finally {
        // 원래 네트워크로 복원
        if (targetNetwork !== originalNetwork) {
          this.switchNetwork(originalNetwork);
        }
      }
  ...
  switchNetwork(network) {
    ...
    // 네트워크별로 적절한 Explorer 생성
    // WorldLand 계열은 BlockScout, 그 외는 Etherscan 계열
    if (network === 'mainnet' || network === 'testnet') {
      this.explorer = new WorldLandExplorerAPI(network);
    } else if (network === 'ethereum') {
      this.explorer = new EtherscanAPI(network);
    } else if (network === 'bsc') {
      this.explorer = new BscScanAPI(network);
    }
    
    consolelog(`네트워크 전환: ${NETWORKS[network].name}`);
  }
